...
namespace SpaceInvaders
{
    public class Engine
    {
        ...
        // adding alien ship shooting
        public virtual void AlienShoot()
        {
            int n = GetRandomNumber(this.alienShips.Count);
            var ship = this.alienShips[n] as AlienShip;
            ship.IsShooting = true;
        }

        private int GetRandomNumber(int count)
        {
            int random = randomGenerator.Next(0, count);
            return random;
        }

        public virtual void Run()
        {
            int alienShootInterval = 0;
            while (true)
            {
                ...
                if (alienShootInterval == 10)
                {
                    this.AlienShoot();
                    alienShootInterval = 0;
                }
